/* ============================================================
   SREE GANESHA MAHAL — Availability Calendar
   HOW TO UPDATE DATES: Edit /data/availability.json
   Format: "YYYY-MM-DD"
   ============================================================ */

let availabilityData = { booked: [], tentative: [] };
...
function renderCalendar(year, month) {
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    if (isPast) {
      className += ' past';
    } else if (availabilityData.booked?.includes(dateStr)) {
      className += ' booked';
      title = 'Booked';
    } else if (availabilityData.tentative?.includes(dateStr)) {
      className += ' tentative';
      title = 'Tentative';
    } else {
      className += ' available';
  ...
  // Click to enquire on available dates
  calEl.querySelectorAll('.cal-day.available').forEach(day => {
    day.style.cursor = 'pointer';
    day.addEventListener('click', () => {
      const date = day.dataset.date;
      // Pre-fill booking form if it exists
      const dateInput = document.querySelector('input[name="event_date"], #event-date');
      if (dateInput) {
        dateInput.value = date;
        document.querySelector('#bookingForm, .form-wrap')?.scrollIntoView({ behavior: 'smooth' });
      } else {
